...
function onTaskExecuteMarketAnalysis(routeStage) {
  if (routeStage.executionResult == "executed") {
    if (!EdocsApi.getAttributeValue("CodPurchase").value || !EdocsApi.getAttributeValue("price").value) throw `Заповніть поля «Код ДК 021:2015» та «Очікувана вартість предмета закупівлі» таблиці  «Перелік матеріалів/послуг»`;
  }
}
