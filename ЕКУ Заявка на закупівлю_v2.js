...
function onTaskExecuteMarketAnalysis(routeStage) {
  if (routeStage.executionResult == "executed") {
    var table = EdocsApi.getAttributeValue("ProcuremenTable");
    if (table && table.value) {
      for (let index = 0; index < table.value.length; index++) {
        if (!table.value[index].find((x) => x.code == "CodPurchase")?.value || !table.value[index].find((x) => x.code == "price")?.value) throw `Заповніть поля «Код ДК 021:2015» та «Очікувана вартість предмета закупівлі» таблиці  «Перелік матеріалів/послуг»`;
      }
    }
  }
}
